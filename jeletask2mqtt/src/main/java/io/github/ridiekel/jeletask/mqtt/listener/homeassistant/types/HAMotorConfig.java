...
public class HAMotorConfig extends HAReadWriteConfig<HAMotorConfig> {
    public HAMotorConfig(HAConfigParameters parameters) {
        ...
        this.put("state_value_template", "{% if value_json.state == 'OFF' %}{% if value_json.current_position == 100 %}closed{% elif value_json.current_position == 0 %}open{% else %}stopped{% endif %}{% else %}{% if value_json.last_direction == 'DOWN' %}closing{% else %}opening{% endif %}{% endif %}");
    }
}
