...
public class NbtCentralUnit implements CentralUnit {
    ...
    private Map<String, ComponentSupport> convertComponentsToMap(Collection<ComponentSupport> components) {
        return components.stream().collect(Collectors.toMap(c -> NbtCentralUnit.this.getIndex(c.getFunction(), c.getId()), java.util.function.Function.identity()));
    }
