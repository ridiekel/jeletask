...
public class MicrosPlusFunctionConfiguration extends ConfigurationSupport<Function, FunctionConfigurable, Integer> {
    ...
    private static final Map<SensorType, StateCalculator<?>> SENSOR_CALCULATORS = Map.of(
            SensorType.TEMPERATURE, new TemperatureStateCalculator(),
            SensorType.LIGHT, new LuxStateCalculator(),
            SensorType.HUMIDITY, new HumidityStateCalculator(),
            SensorType.GAS, new GasStateCalculator(),
            SensorType.TEMPERATURECONTROL, new TemperatureControlStateCalculator(),
            SensorType.PULSECOUNTER, new PulseCounterStateCalculator()
    );
    ...
    public MicrosPlusFunctionConfiguration() {
        super(List.of(
                ...
                new FunctionConfigurable(Function.SENSOR, 20, c -> Optional.ofNullable(SENSOR_CALCULATORS.get(SensorType.from(c))).orElseThrow(() -> new IllegalArgumentException("Cannot find sensor state calculator for component " + c))),
